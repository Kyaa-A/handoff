...
6. **Mark confirmed versus assumed.** Tag what was verified (tests passed, feature checked) separately from what is believed but unverified, so the next session does not trust a stale claim. A long session may have been auto-compacted by the time you write the note, so ground the confirmed list in real evidence (a `git status`, a `git log`, an actual test run) instead of trusting memory. For verification evidence, preserve the exact command and the observed result wording and counts; do not normalize, summarize, or invent them. Exact evidence lets the next session distinguish what the tool actually reported from the handoff writer's interpretation.
