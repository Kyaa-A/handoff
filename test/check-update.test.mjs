...
test('hook and marketplace manifests parse and versions match', async () => {
  ...
  assert.equal(documents['.claude-plugin/plugin.json'].version, '0.4.0');
  assert.equal(documents['.codex-plugin/plugin.json'].version, '0.4.0');
  assert.equal(documents['.agents/plugins/marketplace.json'].plugins[0].version, '0.4.0');
  assert.equal(documents['package.json'].version, '0.4.0');
