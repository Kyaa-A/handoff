...
runs with `workspace-write`, approval policy `never`, and no additional writable
directories. Do not replace these controls with danger-full-access or sandbox
bypass flags. Network access is unavailable under the selected sandbox policy.
